...
export class TaskRepository {
  ...
  findManySummary(filter?: TaskFilter): TaskSummary[] {
    ...
    const rows = this.db
      .query(`SELECT id, project_id, title, status, effort, impact, category, group_key, (context IS NOT NULL) AS has_context, (plan IS NOT NULL) AS has_plan, (description IS NOT NULL) AS has_description, (implementation IS NOT NULL) AS has_implementation, (acceptance_criteria IS NOT NULL) AS has_acceptance_criteria, 0 AS is_blocked, created_at, updated_at FROM tasks ${where}ORDER BY created_at ASC LIMIT ? OFFSET ?`)
      .all(...params) as (Omit<TaskSummary, "has_context" | "has_plan" | "has_description" | "has_implementation" | "has_acceptance_criteria" | "is_blocked"> & { has_context: number; has_plan: number; has_description: number; has_implementation: number; has_acceptance_criteria: number; is_blocked: number })[];
  ...
  insertMany(rows: Omit<TaskRow, "id" | "created_at" | "updated_at">[]): Task[] {
    const stmt = this.db.query(
      "INSERT INTO tasks (id, project_id, title, context, plan, description, implementation, acceptance_criteria, group_key, status, effort, impact, category, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    );
    ...
    for (const row of rows) {
      const id = ulid();
      ids.push(id);
      stmt.run(id, row.project_id, row.title, row.context ?? null, row.plan ?? null, row.description ?? null, row.implementation ?? null, row.acceptance_criteria ?? null, row.group_key ?? null, row.status, row.effort ?? null, row.impact ?? null, row.category ?? null, now, now);
    }
  ...
  updateMany(rows: { id: string; title?: string; context?: string | null; plan?: string | null; description?: string | null; implementation?: string | null; acceptance_criteria?: string | null; group_key?: string | null; status?: string; effort?: string | null; impact?: string | null; category?: string | null }[]): Task[] {
    const now = new Date().toISOString();
    const results: Task[] = [];
    ...
    for (const row of rows) {
      ...
      this.db
        .query("UPDATE tasks SET title = ?, context = ?, plan = ?, description = ?, implementation = ?, acceptance_criteria = ?, group_key = ?, status = ?, effort = ?, impact = ?, category = ?, updated_at = ? WHERE id = ?")
        .run(title, context, plan, description, implementation, acceptance_criteria, group_key, status, effort, impact, category, now, row.id);
