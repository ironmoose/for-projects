...
export function documentRoutes(service: IDocumentService): Hono {
  ...
  app.get("/", async (c) => {
    ...
    const filter: { search?: string; tag?: string; title?: string; favorite?: boolean; folder?: string; entity_type?: string; entity_id?: string; limit: number; offset: number } = { limit, offset };
