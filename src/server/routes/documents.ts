...
export function documentRoutes(service: IDocumentService): Hono {
  ...
  app.get("/", (c) => {
    ...
    const filter: { search?: string; tag?: string; title?: string; favorite?: boolean; folder?: string; project_id?: string; entity_type?: string; entity_id?: string; limit: number; offset: number } = { limit, offset };
