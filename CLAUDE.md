...
- **Never `SELECT *` from tables with an `embedding` column.** `projects`, `tasks`, and `documents` have `vector(768) embedding` columns in Postgres. These are internal (semantic search only) and must never leak through API or MCP. Always use explicit column lists in Postgres `findById`/`findMany` queries. Regression tests in `pg-integration.test.ts` ("Embedding column is not exposed") guard this — do not delete them. SQLite has no embedding column so `SELECT *` is safe there today, but prefer explicit lists for defense in depth.
